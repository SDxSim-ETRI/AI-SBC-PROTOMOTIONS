...
**보조 지연 투입 영상 (`--assist-start-steps`, 2026-07-16 제작)** — 반대
순서 시연: 에피소드 시작부터 α·PD 단독(잔여 근력만)으로 **불안정하게 걷다가**
K스텝째 (1-α)·MF 보조가 투입돼 **정상보행으로 회복**. 15 s off → 15 s on:

```bash
bash tasks/mimic_suit_active_cable_walk_23dof/compare_maniflow_control_newton.sh --record \
    --maniflow-run-dir <run04> --residual-pd-scale 0.5 \
    --assist-start-steps 300 --episode-steps 600
```

α별 대비 (정착 구간 기준, B err6 OFF→ON / OFF 구간 A↔B root 발산):

| α | OFF err6 (A 대비) | ON err6 | OFF reward | OFF xy 발산 | 시각적 대비 |
|---|---|---|---|---|---|
| 0.75 | 0.134 (+4.8%) | 0.134 | 0.842 | 2.9 cm | 거의 안 보임 |
| 0.6 | 0.146 (+14.5%) | 0.135 | 0.817 | 5.1 cm | 약함 |
| **0.5** | **0.157 (+22.6%)** | **0.136** | **0.759** | **8.6 cm** | **뚜렷 — 시연 권장** |

α=0.5 미만은 PD 단독 생존 불가(위 † 행)라 off 구간에서 넘어져 버림 —
"불안정하되 걷는" 연출의 하한은 α≈0.5.

영상 읽는 법(assist 녹화): 토크 패널에서 보조 꺼짐 구간의 빨간 선(B ManiFlow)
은 0에 고정 — "보조 없음"의 명시적 표시. **파란 세로선 = 보조 투입 시점**
(범례 "assist ON"), 회색 점선 = 현재 시각 커서. 정적 플롯
(torque_channels/tracking png)에도 같은 파란 세로선이 표시된다.

영상 읽는 법(가산 보조 녹화, `--assist-beta`): 패널의 의미가 위와 다르다 —
검정 = A 에이전트 토크(보조 없음, 기준선), **빨강 = B 에이전트 토크(보조
받는 중)**, 초록 = 슈트 보조분 β·MF. 즉 **빨강이 검정 아래로 내려가는 것이
"착용자가 덜 힘쓴다"** 는 뜻이다(α 시연 영상에서 빨강이 "보조 토크"였던 것과
반대). `sim_with_torque_rms.mp4`는 같은 3색을 2 s 이동 RMS 포락선으로 그린
버전으로, 진폭 감소가 훨씬 뚜렷하다.
...
## 가산 보조 실험 (2026-08-06) — 게인을 깎지 않고 "능동적" 토크 감소 측정

앞의 α 블렌드는 게인을 강제로 깎아(α·PD) 에이전트 토크를 **수동적으로**
줄인 것이다. 이번에는 반대로 게인은 그대로 두고 보조 토크만 더해서, **보조
덕분에 트래킹 오차가 줄고 그 결과 PD 오차항이 스스로 작아지는** 경로로
에이전트 토크가 감소하는지를 본다:

    τ_hip = τ_agent(full PD, 게인 무감쇠)  +  β·τ_ManiFlow      (가산)

구현: `JointTorqueOverride.engage(1.0)`(게인 무변경 + 주입만 활성화 — DAgger
외란 수집과 같은 경로)에 `--assist-beta β`. 이 모드에서는 B의
`qfrc_actuator` 리드백이 **감쇠되지 않은 온전한 PD 토크 = τ_agent** 이므로
착용자 토크를 직접 계측할 수 있다. 주입 토크(run04 라벨 = substep 평균)와
단위 정의를 맞추기 위해 τ_agent도 substep 평균 리드백
(`get_substep_mean_dof_forces`)으로 기록한다(trace `tau_{a,b}_mean`).

**왜 별도 지표가 필요한가**: 이 방식은 보조 전/후 모두 정상 보행이라 α
시연처럼 "절뚝임 → 회복"의 시각적 대비가 없다. 대신 (a) **τ_agent 자체의
감소**와 (b) **보조 토크가 필요 토크를 얼마나 맞추는가(τ_agent−τ_exo
tracking error)** 로 보인다. env 0의 Agent A는 같은 정책·같은 레퍼런스·같은
시각을 공유하는 무보조 대조군이므로 매 실행이 자체 β=0 기준선을 포함한다.

### β 스윕 (run04, 1200스텝=60 s, receding, N=3, seed 42)

토크는 hip 6채널 substep 평균 RMS[N·m], 파워는 |τ_agent·ω| 평균[W],
보존오차는 rms(τ_agent^B + β·τ_exo − τ_agent^A), ρ = corr(β·τ_exo, τ_agent^A).

| β | 에이전트 토크 (A→B) | Δagent | offload 효율 | \|파워\| (A→B) | Δpower | 보조 RMS | B 총 RMS | 보존오차 | ρ | 중첩모델 예측 | err6 (A→B) | reward (A→B) | 생존 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **0** (대조군) † | 58.9 → 58.7 | −0.3% | 0.00 | 77 → 76 | −0.8% | 0.0 | 58.7 | **11.6** | — | — | 0.1282 → 0.1282 | 0.853 → 0.853 | 완주 |
| **0.3** | 58.7 → 46.1 | **−21.4%** | 0.71 | 76 → 62 | −17.8% | 16.2 | 61.7 | 30.6 | 0.87 | −22.7% | 0.1283 → 0.1310 | 0.853 → 0.849 | 완주 |
| **0.5** | 58.7 → 41.4 | **−29.5%** | 0.59 | 77 → 56 | −26.6% | 26.3 | 65.0 | 43.2 | 0.75 | −27.3% | 0.1283 → 0.1332 | 0.853 → 0.844 | 완주 |
| **0.6** | 58.8 → 40.7 | **−30.8%** | 0.51 | 77 → 56 | −27.7% | 32.3 | 67.8 | 51.1 | 0.67 | −24.9% | 0.1283 → 0.1347 | 0.853 → 0.839 | 완주 |
| **0.8** | 58.8 → 43.4 | −26.2% | 0.33 | 76 → 60 | −22.0% | 45.1 | 75.1 | 66.6 | 0.51 | −10.4% | 0.1282 → 0.1386 | 0.853 → 0.825 | 완주 |
| **1.0** | 58.7 → 55.6 | −5.3% | 0.05 | 77 → **84** | **+9.2%** | 61.2 | 90.2 | 87.0 | 0.42 | +10.3% | 0.1275 → 0.1512 | 0.853 → 0.787 | 완주 |

† 대조군 = 가산 모드 + 주입 0 (`--assist-beta 1.0 --torque-scale 0`).

읽는 법:

- **보조를 넣으면 에이전트 토크가 실제로 줄어든다** (요청의 핵심): β=0.3에서
  −21%, β=0.5에서 −30%, **β=0.6에서 −31%**(58.8 → 40.7 N·m). 관절 파워
  |τ·ω|도 −28%. 게인을 깎지 않고 얻은 감소이므로 "보조 → 오차 감소 → 착용자가
  덜 힘씀"이라는 **능동적** 경로가 성립한다.
- **대조군이 계측 잡음 하한을 준다**: 주입 0에서 A↔B 차이 −0.3%, 보존오차
  11.6 N·m. 즉 두 world 비교의 바닥 잡음이 이 수준이고, β>0의 감소폭은 그보다
  훨씬 크다.
- **β≈0.6이 최적이고 그 위는 역전**: β=0.8 −26%, β=1.0 −5%(파워는 오히려
  **+9%**). 반면 총토크 RMS는 β와 함께 단조 증가(58.7 → 90.2 N·m) — 넣은
  보조가 대체가 아니라 **과구동으로 새기 시작**한다. 보조를 두 배 넣어도
  착용자 부담이 두 배 줄지 않고, 어느 지점부터는 오히려 늘어난다.
- **원인은 보조 토크 정합도 ρ의 붕괴** (= 사용자가 제안한 τ_agent−τ_exo
  tracking error): ρ = corr(β·τ_exo, τ_agent^A)가 0.87(β=0.3) → 0.75 →
  0.67 → 0.51 → 0.42(β=1.0)로 단조 하락하고, τ_exo 정규화 오차
  rms(τ_exo−τ_agent^A)/rms(τ_agent^A)는 0.50 → 0.67 → 0.78 → 0.96 → 1.10으로
  단조 증가한다(β=1.0에서는 오차가 필요 토크 자체보다 크다).
  보조가 커질수록 B의 상태가 학습 매니폴드를 벗어나 예측이 "지금 필요한
  토크"와 어긋나고, PD가 그 위상 오차를 되받아 치기 때문이다.
  중첩모델 1−√(1−2ρr+r²) (r = 보조/에이전트 RMS 비)은 β≤0.5에서 실측과
  1–2%p로 맞고 β=0.6에서 6%p, β≥0.8에서는 크게 어긋난다 — β가 커지면 B의 보행·목표각
  자체가 변해 "B의 필요 토크 = A의 필요 토크" 가정이 깨지므로 **상한이 아니라
  경향 설명용 기준선**으로만 읽을 것(실측이 모델값을 넘는 구간도 있음).
- **대가는 작지만 단조 증가**: β≤0.6에서 err6 +5.0% 이내·reward −1.6% 이내,
  β=1.0에서 err6 +18.6%·reward −7.7%. **생존은 전 구간 안전(모두 60 s 완주)** —
  PD가 온전히 살아 있어 α 블렌드의 낙상 문턱 문제가 아예 없다.
- **채널 편차**: β=0.6에서 hip_adduction_r −38%, flexion_r/rotation_r −31%,
  flexion_l −30%, adduction_l −30%인데 **hip_rotation_l만 −15%**. 채널별 ρ
  (0.55–0.73)와 그대로 대응 — 정합이 나쁜 채널은 보조를 넣어도 덜 준다.
- **α 블렌드와의 대비 (응용 해석)**: α=0.5 블렌드는 총토크를 A와 같게 유지한
  채(명목 보존) hip 토크의 50%를 슈트가 담당하지만 **낙상 문턱**(PD 단독
  생존은 α>0.375)이 있다 — "잔존 근력이 부족한 착용자를 대신 구동"에 대응.
  가산 β=0.6은 총토크를 +15% 늘리는 대신 착용자 토크를 −31% 줄이고 **낙상
  위험이 없다** — "정상 보행자의 근부하 경감(assist-as-needed)"에 대응.

### 보조 off→on 시연 (β=0.6, `--assist-start-steps 300`, 600스텝)

15 s 무보조(주입 0 + 게인 무변경 = A와 물리적으로 동일) → 15 s 보조. 같은
에이전트·같은 에피소드 내 전/후 비교라 A/B 대조보다 더 깨끗하다:

| 구간 | 에이전트 토크 RMS (A / B) | Δ | \|파워\| (A / B) | Δ | err6 (A / B) |
|---|---|---|---|---|---|
| off (300스텝) | 59.53 / 59.40 | −0.2% | 77.1 / 76.7 | −0.5% | 0.1307 / 0.1308 |
| on (300스텝) | 58.24 / **40.06** | **−31.2%** | 75.6 / **56.3** | **−25.5%** | 0.1335 / 0.1419 |

off 구간이 A와 사실상 동일(±0.2%)한 것 자체가 배선 검증이다 — 주입이 0이고
게인이 무변경이면 두 world는 같은 계산을 한다.

```bash
CKPT=<run04 best epoch=0180-val_loss=0.015600.ckpt>
RES=tasks/mimic_suit_active_cable_walk_23dof/maniflow_control_results
# β 스윕 (60 s씩) + 대조군(보조 0)
bash tasks/.../compare_maniflow_control_newton.sh --maniflow-ckpt $CKPT \
    --assist-beta 1.0 --torque-scale 0 --episode-steps 1200 \
    --output $RES/2026-08-06_run04_b000ctl_add60s
for B in 0.3 0.5 0.6 0.8 1.0; do
  bash tasks/.../compare_maniflow_control_newton.sh --maniflow-ckpt $CKPT \
      --assist-beta $B --episode-steps 1200 --output $RES/..._b${B}_add60s
done
# 스윕 요약 표(md)·그림(png)
python tasks/mimic_suit_active_cable_walk_23dof/summarize_assist_beta.py \
    --dirs $RES/2026-08-06_run04_b*_add60s --output $RES/2026-08-06_assist_beta_sweep
# off→on 시연 녹화 (영상 2종 자동 생성)
bash tasks/.../compare_maniflow_control_newton.sh --record --maniflow-ckpt $CKPT \
    --assist-beta 0.6 --assist-start-steps 300 --episode-steps 600
```

산출물(가산 모드에서만 추가 생성):

- `assist_offload_{full,zoom}.png` — 채널별 A 에이전트 토크(검정) / B 에이전트
  토크(빨강) / 보조 β·MF(초록) / B 총합(회색 점선)
- `assist_rms_trace.png` — 2 s 이동 RMS 추이. **투입 시점에서 빨강이 검정
  아래로 내려가는 그림** = 이 실험의 핵심 한 장
- `assist_summary.png` — 채널별 RMS 막대 + 감소율
- `sim_with_torque.mp4`(원파형) / **`sim_with_torque_rms.mp4`(2 s 이동 RMS
  포락선 — 원파형은 20 Hz 진동이 커 진폭 감소가 잘 안 보이므로 시연은 이 쪽
  권장)**. 파란 세로선 = 보조 투입 시점(`assist ON`).
  파일명의 `rms`는 **위 표의 `에이전트 토크 RMS`와 같은 통계를 2 s 슬라이딩
  창으로 계산**했다는 뜻이다(`_rolling_rms_channels()`) — 그래서 화면에서
  검정(A)과 빨강(B) 사이의 간격이 곧 표의 감소율(β=0.6에서 −31%)이다.
  하필 RMS인 이유: hip 토크는 부호가 바뀌어 \|mean\|은 상쇄되고 peak 포락선은
  이상치 한 점에 흔들리는데, RMS는 τ²의 시간평균이라 둘 다 없고 일률·발열
  계열 지표와도 연결된다.
  두 합성 영상 모두 **정면(위) + 정측면(아래) 2뷰**로 구성된다(2880×2160) —
  스텝마다 카메라를 90° 돌려 한 번 더 렌더해 `sim_side.mp4`를 따로 저장하고,
  합성 시 세로로 이어붙인다. 정면만 원하면 `--no-record-side`, 측면 방향은
  `--side-azimuth`(기본 90°).
  **뷰어 화면**: 측면 패스는 백버퍼에만 그리고 버퍼 스왑을 막으므로 창에
  보이는 화면은 정면 뷰로 고정된다(깜빡임 없음). 측면 프레임은
  `viewer.log_image()`로 같은 창의 도킹 패널 `side view (sagittal)`에
  표시되므로 **창 하나에서 두 시점**을 본다. `--viewer`만 켠 경우에도 패널은
  나오고 mp4는 쓰지 않는다.

**다음 수단(가산 방식에서 보조 몫을 더 키우려면)**: 감소폭을 제한하는 것은
게인이 아니라 ρ(위상 정합)이다. 따라서 **β>0 가산 보조 하에서 수집한 데이터로
재학습**(DAgger 3라운드)이 직접적인 수단 — 지금 run04 데이터에는 α 블렌드와
DART 외란 분포만 있고 "보조를 받는 중인 상태"가 없다.
...
| **보조 지연 투입 녹화** (α·PD 단독 15 s → 보조 투입, α=0.75/0.6/**0.5 권장**) | `maniflow_control_results/2026-07-16_run04_a{075,060,050}_assist300_video` |
| **가산 보조 β 스윕** (60 s씩, 대조군 + β 0.3/0.5/0.6/0.8/1.0) | `maniflow_control_results/2026-08-06_run04_b{000ctl,030,050,060,080,100}_add60s` |
| **가산 보조 스윕 요약** (표 md + json + 3패널 png) | `maniflow_control_results/2026-08-06_assist_beta_sweep/assist_beta_sweep.{md,json,png}` |
| **가산 보조 off→on 녹화** (β=0.6, 15 s 무보조 → 15 s 보조; `sim_with_torque_rms.mp4` 권장) | `maniflow_control_results/2026-08-06_run04_b060_add_assist300_video` |
